...
const LightStatus = require("../models/LightStatus");
const LightDevice = require("../models/LightDevice");
const Schedule = require("../models/schedule"); // <<--- thêm
const { authenticate } = require("../middleware/auth");
...
router.get("/:deviceId/next-command", async (req, res) => {
  try {
    ...
    // =========== 1) Tạo các Command từ Schedule nếu có occurrence đã tới ===========
    try {
      // lấy chỉ những schedule liên quan (node thuộc gateway hoặc schedule target chính gateway)
      const schedules = await Schedule.find({
        active: true,
        $or: [
          { deviceId: { $in: nodeIds.length ? nodeIds : ["__NO_MATCH__"] } },
          { deviceId } // schedule targeting gateway itself
        ]
      }).lean();

      const combineDateAndTime = (dateObj, timeHHmm) => {
      ...
      for (const sch of schedules) {
        try {
          // skip if schedule has explicit endDate in past
          if (sch.endDate && new Date(sch.endDate) < now) continue;

          // ensure schedule targets this gateway's nodes or gateway itself
          const targetIsUnderGateway = nodeIds.includes(sch.deviceId);
          const targetIsGateway = sch.deviceId === deviceId;
          if (!targetIsUnderGateway && !targetIsGateway) continue;

          // search occurrences: try today and yesterday (to catch cases where time passed earlier today)
          const tryDates = [new Date(now), new Date(now.getTime() - 24 * 3600 * 1000)];
          let occ = null;

          for (const base of tryDates) {
            if (sch.repeat === 'daily') {
              const candidate = combineDateAndTime(base, sch.time);
              if (candidate <= now && (!sch.lastRun || candidate > new Date(sch.lastRun))) { occ = candidate; break; }
              ...
              if (sch.dayOfMonth && base.getDate() === sch.dayOfMonth) {
                const candidate = combineDateAndTime(base, sch.time);
                if (candidate <= now && (!sch.lastRun || candidate > new Date(sch.lastRun))) { occ = candidate; break; }
              }
            } else if (sch.repeat === 'yearly') {
              if (sch.dayOfMonth && sch.month && (base.getDate() === sch.dayOfMonth) && ((base.getMonth()+1) === sch.month)) {
                const candidate = combineDateAndTime(base, sch.time);
                if (candidate <= now && (!sch.lastRun || candidate > new Date(sch.lastRun))) { occ = candidate; break; }
              }
            } else if (sch.repeat === 'none' && sch.time) {
              // fallback: if someone saved a schedule without repeat but with time -> treat as one-shot if scheduledAt <= now and not run
              const candidate = combineDateAndTime(base, sch.time);
              if (candidate <= now && (!sch.lastRun || candidate > new Date(sch.lastRun))) { occ = candidate; break; }
            }
          }
          ...
          if (occ) {
            // avoid creating duplicate command for same schedule occurrence
            const existingCmd = await Command.findOne({
              'params.sourceSchedule': sch._id,
              scheduledAt: occ,
            ...
            if (!existingCmd) {
              const newCmd = new Command({
                ...
                command: "BRIGHTNESS",
                params: { value: sch.brightness, sourceSchedule: sch._id, sourceGateway: sch.gatewayId || null },
                status: "pending",
                scheduledAt: occ
              });
              await newCmd.save();
              await Schedule.findByIdAndUpdate(sch._id, { lastRun: occ });
              console.log(`[SCHEDULE -> CMD] created command for schedule ${sch._id} occ=${occ.toISOString()}`);
            } else {
              // ensure lastRun updated so we don't recreate
              if (!sch.lastRun || new Date(sch.lastRun) < occ) {
                await Schedule.findByIdAndUpdate(sch._id, { lastRun: occ });
              }
            }
          }
        } catch (e) {
          console.error("[SCHEDULE PROCESS ERROR]", e && e.stack ? e.stack : e);
        }
      }
    } catch (errSchedule) {
      console.error("[SCHEDULE FETCH/PROCESS] error:", errSchedule && errSchedule.stack ? errSchedule.stack : errSchedule);
      // don't fail the whole request because schedule processing failed; continue to return pending commands
    }

    // =========== 2) Now fetch pending Command as before (these will include new commands created above) =============
    // Build conditions:
    const orConditions = [];
    if (nodeIds.length > 0) {
      orConditions.push({ deviceId: { $in: nodeIds } }); // commands targeted at known nodes
